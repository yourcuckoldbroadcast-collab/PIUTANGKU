...
(() => {
  ...
  function render(inner, opts = {}) {
    ...
    $app.innerHTML =
      `<div class="screen${nav ? "" : " no-nav"}">${inner}</div>` +
  ...
  function debtorRow(d, s) {
    ...
    const sub = s.lastActivity ? "Aktivitas " + relTime(s.lastActivity) : (d.tag ? escapeHtml(d.tag) : "Debitur baru");
    return `<button class="row-card" data-act="go" data-go="/peminjam/${d.id}">
      ${avatarHtml(d)}
      <div class="row-main">
        <div class="row-name">${escapeHtml(d.name)} ${d.tag ? `<span class="pill mint" style="font-size:10px">${escapeHtml(d.tag)}</span>` : ""}</div>
    ...
    render(`
      ...
      <div class="donut-card">
        <div class="donut-wrap">${Charts.donut(g.repaymentRate)}
          <div class="donut-center"><div class="dc-pct">${g.repaymentRate}%</div><div class="dc-l">terbayar</div></div>
        </div>
        <div class="legend">
  ...
  /* ---------------------------------------------------------
     ...
     --------------------------------------------------------- */
  function closeScrim() {
    const sc = $app.querySelector(".scrim");
    if (!sc) return;
  ...
  function debtorFormBody(d) {
    const chips = TAGS.map((t) => `<button class="chip-opt${d && d.tag === t ? " active" : ""}" data-act="pick" data-group="tag" data-val="${t}">${t}</button>`).join("");
    const emojis = EMOJIS.map((e) => `<button class="emoji-opt${(d ? d.emoji || "" : "") === e ? " active" : ""}" data-act="pick" data-group="emoji" data-val="${e}">${e || "Aa"}</button>`).join("");
    const swatches = Calc && (Calc.avatarColor ? null : null);
    const palette = ["", "#8FDCC2", "#B8B5FF", "#FFB38A", "#7FB5E6", "#E59ABF", "#9BD49B", "#F2C76B"];
    const sw = palette.map((c) => {
      const active = (d ? d.color || "" : "") === c;
      const style = c ? `background:linear-gradient(135deg,${shade(c, .95)},${shade(c, .72)})` : "background:repeating-linear-gradient(45deg,#EEF1ED,#EEF1ED 5px,#fff 5px,#fff 10px)";
      return `<button class="swatch${active ? " active" : ""}" data-act="pick" data-group="color" data-val="${c}" style="${style}" aria-label="${c || "otomatis"}"></button>`;
    }).join("");
    ...
    return `
      ...
      <div class="field"><label>Label <span class="opt">(opsional)</span></label>
        <div class="chip-pick" data-pickgroup="tag">${chips}</div></div>
      <div class="field"><label>Ikon <span class="opt">(opsional)</span></label>
        <div class="emoji-pick" data-pickgroup="emoji">${emojis}</div></div>
      <div class="field"><label>Warna avatar <span class="opt">(opsional)</span></label>
        <div class="swatches" data-pickgroup="color">${sw}</div></div>
  ...
  function getActiveVal(group) {
    const grp = $app.querySelector(`[data-pickgroup="${group}"]`);
    if (!grp) return "";
    const a = grp.querySelector(".active");
  ...
  async function saveDebtor(id) {
    ...
    const obj = {
      ...
      tag: getActiveVal("tag"),
      emoji: getActiveVal("emoji"),
      color: getActiveVal("color"),
  ...
  $app.addEventListener("click", async (e) => {
    ...
    if (pick) {
      const grp = pick.closest("[data-pickgroup]");
      if (grp) grp.querySelectorAll(".active").forEach((x) => x.classList.remove("active"));
